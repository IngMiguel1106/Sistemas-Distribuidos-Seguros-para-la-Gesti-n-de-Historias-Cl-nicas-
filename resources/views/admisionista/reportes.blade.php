    ...
    <div class="dashboard-container">
        <aside class="sidebar">
            ...
            <ul class="nav-links">
                ...
                <li><a href="{{ route('admisionista.reportes') }}" style="background: rgba(255,255,255,0.1);">📊 Reportes</a>
                </li>
            </ul>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; align-items: end;">
                        ...
                        <div>
                            <button onclick="generarReporte()"
                                style="background: #0d9488; color: white; border: none; padding: 12px 16px; border-radius: 6px; cursor: pointer; width: 100%; font-weight: 600;">
                                📊 Generar Reporte
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        function renderMetricas(metricas) {
            ...
            const metricasData = [{
                    ...
                    valor: metricas.total_consultas || 0,
                    icon: '📋'
                },
                {
                ...
                },
                {
                    ...
                    valor: metricas.total_ingresos || 0,
                    icon: '📥'
                },
                {
                    label: 'Citas Programadas',
                    valor: metricas.total_citas || 0,
                    icon: '📅'
                }
            ];
        ...
        function renderDetalles(detalles) {
            ...
            if (datos.length === 0) {
                tbody.innerHTML =
                    '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #666;">No hay datos para mostrar</td></tr>';
                return;
            }
        ...
        function exportarPDF() {
            ...
            doc.text('REPORTE DEL SISTEMA - MUNDO PIE', 105, 20, {
                align: 'center'
            });

            doc.setFontSize(10);
            doc.setTextColor(100, 100, 100);
            doc.text(`Generado el: ${new Date().toLocaleString('es-ES')}`, 105, 30, {
                align: 'center'
            });
            doc.text(`Usuario: {{ Auth::user()->nombre_usuario }}`, 105, 35, {
                align: 'center'
            });
            ...
            const metricas = [{
                    label: 'Total Consultas',
                    valor: datosReporte.metricas.total_consultas || 0
                },
                {
                    label: 'Pacientes Atendidos',
                    valor: datosReporte.metricas.pacientes_atendidos || 0
                },
                {
                    label: 'Ingresos Registrados',
                    valor: datosReporte.metricas.total_ingresos || 0
                },
                {
                    label: 'Citas Programadas',
                    valor: datosReporte.metricas.total_citas || 0
                }
            ];
            ...
            metricas.forEach((metrica, index) => {
                ...
                doc.text(metrica.valor.toString(), 160, yPosition + 5 + (index * 10), {
                    align: 'right'
                });
            });
            ...
            const headers = [
                ['Fecha', 'Paciente', 'Documento', 'Médico', 'Especialidad', 'Diagnóstico', 'Estado']
            ];
        ...
        function exportarCSV() {
            ...
            datos.forEach(item => {
                csv +=
                    `"${item.fecha}","${item.paciente}","${item.documento || 'N/A'}","${item.medico}","${item.especialidad}","${item.diagnostico}","${item.estado}"\n`;
            });
