    ...
    <div class="dashboard-container">
        ...
        <aside class="sidebar">
            ...
            <ul class="nav-links" style="list-style:none; padding:0;">
                <li style="padding:14px 25px;"><a href="{{ route('medico.index') }}"
                        style="color:#fff; text-decoration:none;">🏠 Inicio</a></li>
                <li style="padding:14px 25px;"><a href="{{ route('medico.consultas') }}"
                        style="color:#fff; text-decoration:none;">📋 Mis Consultas</a></li>
                <li style="padding:14px 25px;"><a href="{{ route('medico.pacientes') }}"
                        style="color:#fff; text-decoration:none;">👥 Mis Pacientes</a></li>
                <li style="padding:14px 25px;"><a href="{{ route('medico.agenda') }}"
                        style="color:#fff; text-decoration:none;">📅 Agenda</a></li>
                <li style="padding:14px 25px;"><a href="{{ route('medico.reportes') }}"
                        style="color:#fff; text-decoration:none;">📊 Reportes</a></li>
            </ul>
            <form method="POST" action="{{ route('logout') }}" style="padding: 0 25px;">
                @csrf
                <button type="button" onclick="logout()"
                    style="background:#dc3545; color:white; border:none; padding:12px; width:100%; border-radius:8px; cursor:pointer; margin-top:20px;">
                    Cerrar Sesión
                </button>
            </form>
        </aside>
        ...
        function cargarConsultasRecientes() {
            fetch('/api/medico/consultas-recientes')
                ...
                .then(data => {
                    ...
                    if (!data.length) {
                        container.innerHTML =
                        '<p style="text-align:center; color:#777;">No hay consultas recientes</p>';
                        return;
                    }
